...
func TestGenerateJoinCodeRevokesPreviousUnusedCode(t *testing.T) {
	store := testStore(t)
	ctx := context.Background()
	group, err := store.CreateGroup(ctx, "hk", 600)
	if err != nil {
	...
	node, err := store.CreateNode(ctx, Node{
		...
		MonthlyQuotaBytes: 100, ThresholdPercent: 80, ResetDay: 1, TrafficMode: TrafficModeBoth,
		Enabled: true, AutoSwitch: true, Priority: 1, PreferredIface: "auto", ReportIntervalSeconds: 60,
	})
	if err != nil {
		t.Fatal(err)
	}
	first, err := store.GenerateJoinCode(ctx, node.ID, time.Hour)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.GenerateJoinCode(ctx, node.ID, time.Hour); err != nil {
		t.Fatal(err)
	}
	if _, err := store.RedeemJoinCode(ctx, first); err == nil {
		t.Fatal("expected previous join code to be revoked when a new one is generated")
	}
}
