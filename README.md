...
当一组 VPS 共用同一个业务域名时，你可以用 quota-dns-router 让 DNS A / AAAA 记录指向当前可用节点。节点流量接近阈值、节点离线、或你手动指定切换目标时，Master 会通过配置的 DNS 服务商（Cloudflare 或 Route53）更新记录，并把结果发送到 Telegram。

项目设计目标是简单、可恢复、易排查：

- 所有运行态数据保存在 Master 本地 SQLite。
- Master 安装后只需要 Telegram Bot Token 和 Telegram 管理员 ID。
- DNS 服务商、Zone、记录、分组、节点、策略和 Agent 安装命令都在 Telegram Bot 内完成。
- Agent 默认只读取 Linux `/proc/net/dev`，不上报代理配置、业务端口或用户流量内容。

## 适用场景

- 多台 Linux amd64 / arm64 VPS 共用一个 DNS A 或 AAAA 记录。
- 需要按月流量额度和阈值自动把域名切到备用节点。
- 希望通过 Telegram 完成运维配置；可选只读 Web 状态面板辅助排查。
...
1. Master 运行 Telegram long polling，管理员通过 Bot 配置 DNS 服务商、Zone、记录、节点和策略。
...
5. 需要切换时，Master 调用 DNS API（Cloudflare 或 Route53），把指定分组的 DNS 记录更新为目标节点 IP。
...
- Cloudflare / Route53 DNS A / AAAA 记录查询与更新。
- 分组、节点、默认策略和节点级策略。
...
- [生产部署检查清单](docs/production-checklist.md) — 上线前逐项确认
- [Telegram 运维命令速查](docs/telegram-ops-cheatsheet.md) — 斜杠命令与按钮对照
- [v0.2.3 Smoke 验收](docs/releases/v0.2.3-smoke.md) — 安装/升级后只读检查（版本号以当前 Release 为准）
- [GitHub Release 编辑指南](docs/guides/gh-cli-release.md) — 使用 `gh` 更新 Release 说明

## 当前限制

- DNS 服务商：Cloudflare（默认）与 Route53；其他服务商待扩展。
- 仅支持 DNS A / AAAA 记录（不支持 CNAME 等）。
- Release 二进制提供 Linux amd64 / arm64。
- 仅支持 Telegram long polling，不支持 webhook。
- Web 界面仅为只读状态面板，不提供完整配置 UI。
- 不提供 Docker 管理。
...
```bash
QDR_TELEGRAM_BOT_TOKEN="你的BotToken" QDR_TELEGRAM_ADMIN_ID="你的管理员ID" bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/install-master.sh) --yes
```

安装器默认使用 `QDR_INSTALL_MODE=binary`：

- 从 GitHub Releases 下载 `v0.2.3.5` 二进制包。
- 不安装 Go。
- 不执行 `git clone`。
- 不执行源码构建。
- 下载 `SHA256SUMS` 并校验归档。
...
```bash
QDR_INSTALL_MODE=source bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/install-master.sh)
```

源码模式适合非 amd64 或需要自行构建的环境，需要 Go 和较多磁盘空间。
源码模式默认构建 `v0.2.3.5`，需要指定其他引用时可以设置 `QDR_BRANCH`。
...
1. 配置 Master 公网地址。
2. 配置 DNS 服务商（Cloudflare Token + Zone，或 Route53 Hosted Zone）。
3. 配置 DNS A 记录。
4. 创建分组。
5. 添加节点。
6. 生成 Agent 安装命令。
...
在 `master.env` 中设置 `QDR_DNS_PROVIDER=route53` 与 `QDR_AWS_REGION`，凭证通过 AWS 环境变量或 IAM Role 提供。Telegram 向导中跳过 Token，直接选择 Hosted Zone。
...
每个分组对应一条 DNS A 或 AAAA 记录。推荐使用清晰的业务记录名，例如：
...
当当前节点达到阈值且存在可用目标时，Master 会更新 DNS 记录。没有可用目标时只发送通知，不会盲目改 DNS。

## 手动切换

你可以在 Telegram 主菜单进入“手动切换”，选择分组和目标节点。手动切换会写入切换历史，触发 DNS 记录更新，并发送结果通知。
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/uninstall-master.sh) --yes
```

卸载 Agent：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/uninstall-agent.sh) --yes
```

完全清理：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/uninstall-master.sh) --yes --purge
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/uninstall-agent.sh) --yes --purge
```
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/smoke.sh) master
```

Agent：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3.5/scripts/smoke.sh) agent
```
...
- DNS 未切换：检查 DNS 服务商凭证、Zone / Hosted Zone、记录类型与节点 IP 是否匹配、节点在线状态和分组 cooldown。
