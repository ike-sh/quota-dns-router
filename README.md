...
安装脚本会自动安装依赖、准备 Go 工具链、下载 GitHub 源码、构建 `qdr-master` 并写入 systemd。Master 安装阶段只询问 Telegram Token 和管理员 ID；Cloudflare、DNS、节点、阈值都在 Telegram 中继续配置。
...
2. /cf
3. /groups add hk
4. /dns set hk hk.example.com ttl=120 proxied=false
5. /nodes add hk-01 1.1.1.1 hk quota=1000GB threshold=80 mode=both priority=10
6. /nodes add hk-02 2.2.2.2 hk quota=1000GB threshold=80 mode=both priority=20
7. /agent install hk-01
```

`/setup` 或 `/start` 会给出同样的初始化向导，并显示当前还缺少哪些配置。
...
- 当前风险：当前需要优先处理的问题，例如 Master URL 仍是本机地址、Token 未配置、Zone 未验证、DNS IP 不匹配节点、没有可用切换目标、节点达到阈值、节点离线、自动切换关闭或正在 cooldown。
...
```bash
QDR_MASTER_API_URL=https://master.example.com bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/main/scripts/install-agent.sh) --join xxxxx
```

如果已经通过环境变量提供了 Master 地址，Agent 安装模板是：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/main/scripts/install-agent.sh) --join <Telegram生成的join_code>
```

Agent raw 安装脚本也支持显式传入 Master 地址：
...
Agent 会使用加入码向 Master 拉取 Agent ID、Agent Token、节点名、分组、上报周期和网卡配置，并写入 `/etc/quota-dns-router/agent.env`。
