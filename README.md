...
- [v0.2.3 Smoke 验收](docs/releases/v0.2.3-smoke.md) — 安装/升级后只读检查
...
```bash
QDR_TELEGRAM_BOT_TOKEN="你的BotToken" QDR_TELEGRAM_ADMIN_ID="你的管理员ID" bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/install-master.sh) --yes
```

安装器默认使用 `QDR_INSTALL_MODE=binary`：

- 从 GitHub Releases 下载 `v0.2.3` 二进制包。
- 不安装 Go。
- 不执行 `git clone`。
- 不执行源码构建。
- 下载 `SHA256SUMS` 并校验归档。
...
```bash
QDR_INSTALL_MODE=source bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/install-master.sh)
```

源码模式适合非 amd64 或需要自行构建的环境，需要 Go 和较多磁盘空间。
源码模式默认构建 `v0.2.3`，需要指定其他引用时可以设置 `QDR_BRANCH`。
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/uninstall-master.sh) --yes
```

卸载 Agent：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/uninstall-agent.sh) --yes
```

完全清理：

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/uninstall-master.sh) --yes --purge
bash <(curl -fsSL https://raw.githubusercontent.com/ike-sh/quota-dns-router/v0.2.3/scripts/uninstall-agent.sh) --yes --purge
```
